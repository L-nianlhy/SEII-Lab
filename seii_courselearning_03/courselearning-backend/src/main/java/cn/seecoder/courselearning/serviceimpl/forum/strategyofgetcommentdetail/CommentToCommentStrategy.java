 ...
 */
public class CommentToCommentStrategy implements StrategyOfGettingCommentDetail {
    ...
    public String getRepliedContent(CommentServiceImpl commentService, Comment comment) {
        CommentMapper commentMapper=commentService.getCommentMapper();

        //获取评论所针对的评论
        Comment repliedComment=commentMapper.selectByPrimaryKey(comment.getRepliedId());
    ...
    public UserVO getRepliedUser(CommentServiceImpl commentService, Comment comment) {
        ...
        return userService.getUser(repliedComment.getUserId());
    }
