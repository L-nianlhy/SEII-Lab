    ...
    setLikeOrDislike(courseId) {
      ...
      getCourseById({ courseId, uid }).then(res => {
        if (res.liked) {
          ...
          // TODO Add your code here
          setCourseDislike(uid, courseId).then(res => {//后端的这个方法，原本代码中创建CourseVO时，对当前用户是否点赞信息的处理不正确，需要修改
            if (res.code === 1) {
              this.snackBarMsg = res.msg;
              this.snackBarColor = "success";
              this.showSnackBar = true;
              // for(var i=0;i<this.hotCourseList.length;i++){
              //   if(this.hotCourseList[i].id===courseId){
              //     this.hotCourseList[i].liked=false;
              //   }
              // }
              this.fetchData();
            }else{
          ...
          setCourseLike(uid, courseId).then(res => {
            if (res.code === 1) {
              ...
              // for(var i=0;i<this.hotCourseList.length;i++){
              //   if(this.hotCourseList[i].id===courseId){
              //     this.hotCourseList[i].liked=true;
              //   }
              // }
